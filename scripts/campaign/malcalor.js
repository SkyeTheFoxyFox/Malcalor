...
const malcalorgen = extend(PlanetGenerator, {
    rawHeight(position){
    ...
    generate(tiles, sec){
        ...
        const Room = {
            ...
            connect(to){
                ...
                malcalorgen.brush(malcalorgen.pathfind(this.x, this.y, to.x, to.y, tile => (tile.solid() ? 5 : 0) + malcalorgen.noiseOct(tile.x, tile.y, 1, 1, 1 / nscl) * 60, Astar.manhattan), stroke);
            }
        };
...
malcalorgen.basegen = new BaseGenerator();
malcalorgen.scl = 5;
malcalorgen.waterOffset = 0.07;
malcalorgen.water = 2 / malcalorgen.arr[0].length;
Events.run(ClientLoadEvent, () => {

}) 
const malcalor = new Planet("Malcalor", Planets.erekir, 1, 1);
malcalor.accessible = true;
malcalor.generator = malcalorgen;
malcalor.meshLoader = () => new HexMesh(malcalor, 5);
malcalor.orbitRadius = 25;
malcalor.orbitTime = 250;
malcalor.rotateTime = 100;
malcalor.bloom = false;
malcalor.alwaysUnlocked = true;
malcalor.defaultCore = Blocks.coreShard
malcalor.startSector = 15;
malcalor.localizedName = "Malcalor";
malcalor.hasAtmosphere = true;
malcalor.atmosphereRadOut = 0.25;
malcalor.atmosphereRadIn = 0.05;
malcalor.hiddenItems.addAll(Items.erekirItems).addAll(Items.serpuloItems);
malcalor.atmosphereColor = Color.valueOf("f3a8ff");
//const ruins = new SectorPreset("ruins", malcalor, 15);
//ruins.difficulty = 5;
